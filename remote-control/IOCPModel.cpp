...
{    
	...
	while (WAIT_OBJECT_0 != WaitForSingleObject(pIOCPModel->m_hShutdownEvent, 0))
	{
		...
		{  	
			...
			{  
				pIOCPModel->_ShowMessage( "客户端 %s:%d 断开连接.",inet_ntoa(pSocketContext->m_ClientAddr.sin_addr), ntohs(pSocketContext->m_ClientAddr.sin_port) );
...
bool CIOCPModel::_DoAccpet( PER_SOCKET_CONTEXT* pSocketContext, PER_IO_CONTEXT* pIoContext )
{
	...
	this->_ShowMessage( "客户额 %s:%d 信息.",inet_ntoa(ClientAddr->sin_addr), ntohs(ClientAddr->sin_port));
  ...
  // 显示客户端
	CremoteControlDlg* pMain = (CremoteControlDlg*)m_pMain;
  pMain->SetNewClient(pIoContext->m_sockAccept, ClientAddr->sin_addr, ntohs(ClientAddr->sin_port), pNewSocketContext);
...
bool CIOCPModel::_DoRecv( PER_SOCKET_CONTEXT* pSocketContext, PER_IO_CONTEXT* pIoContext )
{
  ...
  DWORD nMessLen = *(DWORD*)(pIoContext->m_wsaBuf.buf);
  BYTE nMessType = -1;
  if(nMessLen > 0)
  {
    nMessType = *(BYTE*)(pIoContext->m_wsaBuf.buf + sizeof(DWORD));
    switch(nMessType)
    {
      case emMessType::heartbeat:
      {
	      if( m_pMain!=NULL )
	      {
          pMain->GetHeartBeat(pIoContext->m_sockAccept);
	        this->_ShowMessage( "收到  %s:%d 心跳包",inet_ntoa(ClientAddr->sin_addr), ntohs(ClientAddr->sin_port));
	      }	
      }
        break;
      case emMessType::cmdtext:
	      if( m_pMain!=NULL )
	      {
          pMain->GetCmdText(pIoContext->m_sockAccept, pIoContext->m_wsaBuf.buf + sizeof(DWORD) + sizeof(BYTE));
	        this->_ShowMessage( "收到  %s:%d cmd数据",inet_ntoa(ClientAddr->sin_addr), ntohs(ClientAddr->sin_port));
	      }	
          break;
      case emMessType::screenbmp:
	      if( m_pMain!=NULL )
	      {
          pMain->GetScreenBmp(pIoContext->m_sockAccept, pIoContext->m_wsaBuf.buf + sizeof(DWORD) + sizeof(BYTE), nMessLen - sizeof(BYTE));
	        this->_ShowMessage( "收到  %s:%d 屏幕数据",inet_ntoa(ClientAddr->sin_addr), ntohs(ClientAddr->sin_port));
	      }	
          break;
      case emMessType::processtext:
	      if( m_pMain!=NULL )
	      {
          pMain->GetProcessInfo(pIoContext->m_sockAccept, pIoContext->m_wsaBuf.buf + sizeof(DWORD) + sizeof(BYTE));
	        this->_ShowMessage( "收到  %s:%d 进程数据",inet_ntoa(ClientAddr->sin_addr), ntohs(ClientAddr->sin_port));
	      }	
          break;
      case emMessType::processkillsecc:
	      if( m_pMain!=NULL )
	      {
          pMain->GetProcessKill(pIoContext->m_sockAccept, emMessType::processkillsecc);
	        this->_ShowMessage( "收到  %s:%d 结束进程反馈",inet_ntoa(ClientAddr->sin_addr), ntohs(ClientAddr->sin_port));
	      }	
          break;
      case emMessType::processkillfail:
	      if( m_pMain!=NULL )
	      {
          pMain->GetProcessKill(pIoContext->m_sockAccept, emMessType::processkillsecc);
	        this->_ShowMessage( "收到  %s:%d 结束进程反馈",inet_ntoa(ClientAddr->sin_addr), ntohs(ClientAddr->sin_port));
          ...
          break;
      case emMessType::filedisk:
	      if( m_pMain!=NULL )
	      {
          pMain->GetFileDisk(pIoContext->m_sockAccept, pIoContext->m_wsaBuf.buf + sizeof(DWORD) + sizeof(BYTE));
	        this->_ShowMessage( "收到  %s:%d 收到硬盘信息",inet_ntoa(ClientAddr->sin_addr), ntohs(ClientAddr->sin_port));
	      }	
          break;
      case emMessType::filelist:
	      if( m_pMain!=NULL )
	      {
          pMain->GetFileList(pIoContext->m_sockAccept, pIoContext->m_wsaBuf.buf + sizeof(DWORD) + sizeof(BYTE));
	        this->_ShowMessage( "收到  %s:%d 收到文件信息",inet_ntoa(ClientAddr->sin_addr), ntohs(ClientAddr->sin_port));
	      }	
          break;
      default:
	      this->_ShowMessage( "收到  %s:%d 不知类型的信息",inet_ntoa(ClientAddr->sin_addr), ntohs(ClientAddr->sin_port));
        break;
    }
  }
...
////////////////////////////////////////////////////////////////////
// 获得本机的IP地址
CStringA CIOCPModel::GetLocalIP()
{
	// 获得本机主机名
...
void CIOCPModel::_ShowMessage(const CStringA szFormat,...) const
{
	// 根据传入的参数格式化字符串
	CStringA   strMessage;
	va_list   arglist;
...
bool CIOCPModel::HandleError( PER_SOCKET_CONTEXT *pContext,const DWORD& dwErr )
{
	...
	{  	
	    ...
	    if( m_pMain!=NULL )
	    {
        pMain->ReleaseSocketInfo(pContext->m_Socket);
	    }	
	...
	{
		this->_ShowMessage( _T("完成端口操作出现错误。错误代码：%d"),dwErr );
		return true;
	}
}
